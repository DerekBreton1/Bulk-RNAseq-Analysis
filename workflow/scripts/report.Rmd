...
# Use only transcript_id and gene_id columns
tx2gene <- tx2gene[, c("transcript_id", "gene_id", "gene_name")]

# Save as CSV file
#write.csv(tx2gene, "tx2gene_updated.csv", row.names = FALSE)
head(tx2gene)
```
...
pca_plot <- as_tibble(pca$x) %>%
    ...
    geom_point() +
    labs(title = "PCA Plot of Abundance",
         color = 'Condition',
         x = x_var,
         y = y_var)
pca_plot
...
# Create DESeq Dataset from txi.kallisto and samples data
```{r}
...
labeled_results <- as_tibble(res) %>%
  #Add a column for genes
  dplyr::mutate(genes = rownames(res), .before = 1) %>%
  #Assign UP, DOWN, or NS conditionally
